...
const DialogRoot = () => {
  const { currentContent, goToPrevStep, reset } = useDialog();

  return (
    <Dialog.Root onOpenChange={() => setTimeout(reset, 300)}>
      ...
      <Dialog.Portal>
        ...
        <Dialog.Content className="fixed left-[50%] top-[50%] z-[100] w-[90%] max-w-screen-lg translate-x-[-50%] translate-y-[-50%] rounded-3xl bg-test-bg focus:outline-none data-[state=closed]:animate-[dialog-content-hide_300ms] data-[state=open]:animate-[dialog-content-show_300ms]">
          <Dialog.Title className="shadow-bottom flex w-full items-center justify-between rounded-t-3xl bg-test-bg p-4 font-medium text-test-hint">
            <ArrowDownIcon
              className="rotate-90 text-test-button"
              onClick={goToPrevStep}
            />
            <p>Добавление товара</p>
            <Dialog.Close
              className="outline-none"
              onClick={() => setTimeout(reset, 300)}
            >
              <CloseIcon className="size-7" />
            </Dialog.Close>
          </Dialog.Title>
          <ScrollArea.Root className="relative max-h-[450px] min-h-[300px] overflow-scroll scrollbar-hide">
          ...
          <div className="p-4 text-center text-sm text-test-hint">
